...
### vue-router就是利用了这一部分，分别是hash 和 history 两种模式

这两种模式，理解清楚后，对vue-route就能理解很大一部分

先说单页面应用做了什么吧，url全程成为资源定位符，当我通过网络请求向着服务器发出资源请求，服务器响应我响应的资源，这个就是传统的多页面的形式。

单页面就是无需向服务器请求多个资源，只需要一个，在这一个上面进行修改。

### vue-router是怎么实现的

1. hash模式：
   url `#` 后面的内容被称为hash部分，这一部分本来是用来对页面中的锚点，或者说是定位点进行定位的，你可以想成本页面的超链接。

   vue-router 的hash模式就是利用这个hash部分变化的事件，去修改页面渲染的内容，hash部分的变化不会引起资源请求的，因为这个是在本页面的定位用的。
3. history模式：
   之所以取名叫这个，因为是利用了html5的 history api, 这个api是浏览器的历史记录相关的api，不需要多了解，清楚作用就可以了

   和hash部分差不多，路由和渲染内容绑定的到一起，url的变化，就渲染了相应的内容，大概就是你触发了加载资源，就创建一个历史记录的条目，然后切换就行了

   历史条目是浏览器保存的你访问过的页面，再次访问历史条目，不会发送到服务器去重新请求，会使用这个条目。切换条目同样也会触发事件。
   
   对比之前的hash还是有一些好处的，具体的就不展开说了。 这个到现在还没结束，尽管我们切换条目没什么问题，不会发送到服务器，但是你不可以刷新，因为你刷新浏览器会触发请求，还有些其他的行为也会如此

   请记得这个条目的操作，是改变了url 但是不会请求资源的。

   所以需要在服务器设置，请求任何资源，都只返回这个index.html(这里假设你用的index哈），页面的渲染内容的变化，交给前端。
